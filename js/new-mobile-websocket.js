...
for (i = 0; i < parminform.length; i++) {
    ...
    if (!(parminform[i].OnMsg.includes("bit"))) {
        ...
        spKeyCode = "SP" + parminform[i].InstanceNo;
        ...
        try {
            ...
            if (document.getElementById(keyCode)) {
                ...
                document.getElementById(keyCode).textContent = val.toString();
                document.getElementById(spKeyCode).value = val;
            }
        ...
        try {
            ...
            if (document.getElementById(keyCode + "M")) {
                ...
                document.getElementById(keyCode + "M").textContent = (parseInt(val / 60)).toString();

                //   document.getElementById(keyCode + "S").value = val % 60;
                document.getElementById(keyCode + "S").textContent = (val % 60).toString();
...
function onMessage(evt) {
    ...
    for (i = 0; i < aiinform.length; i++) {
        ...
        if (document.getElementById("A" + aiinform[i].InstanceNo)) {
            document.getElementById("A" + aiinform[i].InstanceNo).innerHTML = val;
        }
    }
...
function btnDownload() {
    ...
    for (let keyVal in parameterobject) {
        ...
        if (parameterobject[keyVal].OnMsg === "bitbit") {
            try {
               ...
               // console.log ( " bitbit ", instanceNo , keyVal , parmvalues[instanceNo] );
            ...
            try {
               // console.log ( "ten ", instanceNo , keyVal , parmvalues[instanceNo] );
                // parmvalues[instanceNo] = parmvalues[instanceNo]*10;
            } catch (error) {
                console.log(keyVal, "Not Found");
                continue;
            }
            ...
            try {
             //   console.log ( "100f : ", instanceNo , keyval , parmvalues[instanceNo] );
                parmvalues[instanceNo] =  parmvalues[instanceNo]*100;
            } catch (error) {
                console.log(keyVal, "Not Found");
                continue;
            }
        } else if (parameterobject[keyVal].OnMsg === "secs") {
          //  console.log ( "secs : ", instanceNo , keyVal , parmvalues[instanceNo] );
            

        }
            
        else { // Normal            
         //   console.log ( "Normal : ", instanceNo , keyVal , parmvalues[instanceNo] );
        }
   ...
   // console.log ( "parmMessage :" , message);
...
function sliderfunc(element) {
    ...
    //let id = element.id;
    //let instanceNo = document.getElementById(id).name.replace(/[^0-9]/g, '');
    
    let instanceNo = element.id.replace(/[^0-9]/g, '');
    let value = parseInt (element.value);
    //console.log ( instanceNo, value );  //index = instanceNo 
  
    //console.log ("before :" , parmvalues [instanceNo]);
    
    if ( parameterobject["P"+instanceNo].OnMsg.includes("ten")){
   ...
   // console.log ( instanceNo, value  ,parmvalues[instanceNo]);
    
    if ( parameterobject["P"+instanceNo].OnMsg.includes("secs")){
         
         let upper =  Math.floor (value / 60) ;
         let lower =  value % 60 ;     
         document.getElementById("P" + instanceNo + "M").textContent = upper.toString();
         document.getElementById("P" + instanceNo + "S").textContent = lower.toString();
         
     }
     
     else {
          document.getElementById("P" + instanceNo).textContent = value.toString();
     }
    ...
    // parmvalues [index] = (parameterobject[index].OnMsg.includes("ten") ) ? (10*val) : val;
    // document.getElementById("P" + instanceNo).innerHTML = parameterobject[instanceNo].TagName + " : " +  value.toString() + " ";

   

    // console.log ( parminform[index].TagName, val,parminform[index].UnitName );
   // console.log ("after :" , parmvalues [instanceNo]);
